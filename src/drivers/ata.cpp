...
AdvancedTechnologyAttachment::AdvancedTechnologyAttachment(bool master, types::uint16_t portBase)
:   dataPort(portBase),
...
void AdvancedTechnologyAttachment::Write28(types::uint32_t sectorNum, types::uint8_t* data, types::uint32_t count)
{
    if(sectorNum > 0x0FFFFFFF)
        return;
    if(count > 512)
