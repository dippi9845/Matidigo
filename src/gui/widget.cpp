...
void Widget::OnMouseDown(types::int32_t x, types::int32_t y, types::uint8_t button)
{
    if(Focussable)
        GetFocus(this);
}
...
void Widget::OnMouseUp(types::int32_t x, types::int32_t y, types::uint8_t button)
{
}
...
void CompositeWidget::OnMouseDown(int32_t x, int32_t y, types::uint8_t button)
{
...
void CompositeWidget::OnMouseUp(int32_t x, int32_t y, types::uint8_t button)
{
    for(int i = 0; i < numChildren; ++i)
        if(children[i]->ContainsCoordinate(x - this->x, y - this->y))
        {
